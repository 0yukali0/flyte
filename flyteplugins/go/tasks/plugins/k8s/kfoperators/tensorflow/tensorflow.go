...
func (tensorflowOperatorResourceHandler) CommonLabels() (map[string]string, map[string]string) {
...
func (tensorflowOperatorResourceHandler) MutateResource(ctx context.Context, object client.Object, taskTmpl *core.TaskTemplate) error {
	cfg := common.GetConfig().BatchScheduler
	tfjob := object.(*kubeflowv1.TFJob)
	id := taskTmpl.Id
	if schedulerName := cfg.Scheduler; schedulerName == yunikorn.Yunikorn {
		defualtCfg := cfg.Default.YunikornConfig
		yunikorn.CreateCommLabels("tf", id.Project, id.Domain, tfjob.ObjectMeta.Namespace, defualtCfg.Queue)
		/*
		if err := yunikorn.MutateTFJob(tfjob, defualtCfg.Parameters); err != nil {
			return err
		}
		*/
	}
	if schedulerName := cfg.Scheduler; schedulerName == kueue.Kueue {
		...
		kueue.CreateCommLabels("tf", id.Project, id.Domain, tfjob.ObjectMeta.Namespace, defualtCfg.Queue)
	}
	return nil
}
